      ...
      <main className="max-w-[90rem] mx-auto w-full px-4 sm:px-6 lg:px-8 py-10">
        ...
        {loading && projects.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
            <button
              onClick={() => setShowCreate(true)}
              className="flex flex-col items-center justify-center bg-slate-50/50 border-2 border-dashed border-slate-300 rounded-xl hover:border-brand-400 hover:bg-brand-50/50 transition-all group text-center cursor-pointer p-6 min-h-[210px]"
            >
              <div className="bg-white rounded-full flex items-center justify-center shadow-sm border border-slate-200 group-hover:border-brand-300 group-hover:scale-110 transition-transform w-14 h-14 mb-4">
            ...
            {projects.map((p) => (
              <ProjectCard key={p.id} project={p} onDelete={(id) => setDeleteConfirm(id)} viewMode="grid" />
            ))}
          </div>
        ) : (
          // ================= LIST VIEW (TABLE) =================
          <div className="flex flex-col gap-6 w-full">
            <button
              onClick={() => setShowCreate(true)}
              className="flex items-center gap-4 bg-slate-50/50 border-2 border-dashed border-slate-300 rounded-xl hover:border-brand-400 hover:bg-brand-50/50 transition-all group text-left cursor-pointer p-4 h-[76px]"
            >
              <div className="bg-white rounded-full flex items-center justify-center shadow-sm border border-slate-200 group-hover:border-brand-300 group-hover:scale-110 transition-transform w-10 h-10 flex-shrink-0">
                <svg className="w-5 h-5 text-slate-400 group-hover:text-brand-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                </svg>
              </div>
              <span className="font-semibold text-slate-600 group-hover:text-brand-700 text-lg">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
              {/* Table Header */}
              <div className="grid grid-cols-[4fr_3fr_3fr_3fr_48px] items-center gap-4 py-3 px-6 border-b border-slate-200 text-xs font-semibold text-slate-500 text-left">
                <div>Project Title</div>
                <div>Date Created</div>
                <div>Model</div>
                <div>Number of Reqs</div>
                <div></div>
              </div>
              ...
              <div className="flex flex-col">
                {projects.map((p) => (
                  <ProjectCard key={p.id} project={p} onDelete={(id) => setDeleteConfirm(id)} viewMode="list" />
                ))}
              </div>
            </div>
          </div>
