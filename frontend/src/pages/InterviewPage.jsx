...
export default function InterviewPage() {
  ...
  return (
    <div className="h-screen bg-slate-50 flex flex-col overflow-hidden">
      ...
      {/* --- HEADER --- */}
      <div className="flex-none z-50 bg-white border-b border-slate-200">
        <Navbar projectName={project?.app_name} backTo="/" backLabel={t('dashboard')} />
        
        {/* Model badge bar */}
        {project && (
          <div className="px-6 py-2 bg-slate-50 flex border-t border-slate-100">
            <ModelSelector
      ...
      <div className="flex-1 flex flex-row overflow-hidden">
        ...
        <div className="w-3/4 flex flex-col bg-white border-r border-slate-200 relative">
          
          <div className="flex-1 overflow-y-auto px-4 py-6 w-full scrollbar-hide">
            ...
            <div className="max-w-4xl mx-auto">
              <form onSubmit={handleSend} className="flex gap-3 items-end">
                <textarea
                  ...
                  className="flex-1 border border-slate-300 rounded-xl px-4 py-3 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent max-h-32 overflow-y-auto text-start bg-slate-50"
                ...
                <button
                  ...
                  className="p-3.5 bg-brand-600 text-white rounded-xl hover:bg-brand-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex-shrink-0"
                >
        ...
        <div className="w-1/4 bg-slate-50 flex flex-col h-full overflow-y-auto">
          <div className="p-6">
            
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-medium text-slate-800">Studio</h2>
              {/* Optional: Show dot indicator when SRS is ready to generate */}
              {isSaturated && (
                <span className="flex h-3 w-3 relative" title="Ready for SRS Generation">
            ...
            {/* SRS Info Banner moved to sidebar context */}
            {isSaturated && (
              <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-xl text-xs text-green-800 flex items-start gap-2">
                 <svg className="w-4 h-4 text-green-600 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                </svg>
                <span><strong>{t('info_complete')}</strong> {t('ready_generate')}</span>
              </div>
            )}

            {/* NotebookLM Style Buttons */}
            <div className="flex flex-col gap-3">
              
              <button
                onClick={handleGenerateSRS}
                disabled={generating}
                className="w-full flex items-center justify-between px-4 py-4 bg-white border border-slate-200 rounded-2xl hover:bg-slate-100 hover:shadow-sm transition-all disabled:opacity-60 group text-left"
              >
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-blue-50 text-blue-600 rounded-lg">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                  </div>
                  <span className="font-medium text-slate-700 text-sm">{generating ? t('generating') : t('generate_srs') || 'Generate SRS'}</span>
                </div>
                {generating ? (
                  ...
                  <svg className="w-4 h-4 text-slate-400 group-hover:text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                )}
              </button>

              <button className="w-full flex items-center justify-between px-4 py-4 bg-white border border-slate-200 rounded-2xl hover:bg-slate-100 hover:shadow-sm transition-all group text-left">
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-medium text-slate-700 text-sm">Activity Diagram</span>
                </div>
                <svg className="w-4 h-4 text-slate-400 group-hover:text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
              </button>
              ...
              <button className="w-full flex items-center justify-between px-4 py-4 bg-white border border-slate-200 rounded-2xl hover:bg-slate-100 hover:shadow-sm transition-all group text-left">
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-medium text-slate-700 text-sm">Usecase Diagram</span>
                </div>
                <svg className="w-4 h-4 text-slate-400 group-hover:text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
              </button>
              ...
              <button className="w-full flex items-center justify-between px-4 py-4 bg-white border border-slate-200 rounded-2xl hover:bg-slate-100 hover:shadow-sm transition-all group text-left">
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-medium text-slate-700 text-sm">Class Diagram</span>
                </div>
                <svg className="w-4 h-4 text-slate-400 group-hover:text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
              </button>
              ...
              <button className="w-full flex items-center justify-between px-4 py-4 bg-white border border-slate-200 rounded-2xl hover:bg-slate-100 hover:shadow-sm transition-all group text-left">
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-medium text-slate-700 text-sm">Sequence Diagram</span>
                </div>
                <svg className="w-4 h-4 text-slate-400 group-hover:text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
              </button>
